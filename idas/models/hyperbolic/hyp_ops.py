...
Based on the implementation available at: https://github.com/dalab/hyperbolic_nn
...
def mob_add(u, v, c):
    numerator = (1.0 + 2.0 * c * np.dot(u, v) + c * linalg.norm(v) ** 2) * u + (1.0 - c * linalg.norm(u) ** 2) * v
    denominator = 1.0 + 2.0 * c * np.dot(u, v) + c ** 2 * linalg.norm(v) ** 2 * linalg.norm(u) ** 2
...
def poinc_dist_sq(u, v, c):
    sqrt_c = np.sqrt(c)
    atanh_x = sqrt_c * linalg.norm(mob_add(-u, v, c))
...
def lambda_x(x, c):
    return 2. / (1 - c * linalg.norm(x) ** 2)


# ================================================================================================================== #

def unit_speed_geo(x, v, t, c):
    second_term = np.tanh(np.sqrt(c) * t / 2) / (np.sqrt(c) * linalg.norm(v)) * v
    return mob_add(x, second_term, c)


def exp_map_x(x, v, c):
    second_term = np.tanh(np.sqrt(c) * lambda_x(x, c) * linalg.norm(v) / 2) / (np.sqrt(c) * linalg.norm(v)) * v
    return mob_add(x, second_term, c)
...
def log_map_x(x, y, c):
    diff = mob_add(-x, y, c)
    lam = lambda_x(x, c)
    return 2. / (np.sqrt(c) * lam) * np.arctanh(np.sqrt(c) * linalg.norm(diff)) / (linalg.norm(diff)) * diff
...
# 
#     v1_instance = v1_instance * 0.59999 / linalg.norm(v1_instance)
#     v2_instance = v2_instance * 0.99 / linalg.norm(v2_instance)
# 
...
#                sess.run(
#                    [mat_mul_op, exp_map_x_op, log_map_x_op, lambda_x_op, mob_scalar_mul_op, mob_dif_op, poinc_dist_op,
#                     eucl_dist_op],
#                     feed_dict={
#                         v1: v1_instance,
#                         v2: v2_instance,
...
#                     })
# 
#         assert linalg.norm(mat_mul_v - mob_mat_mul(M_instance.T, v1_instance.reshape([-1]), c)) < 1e-8
# 
#         assert linalg.norm(exp_map_x_v - exp_map_x(v1_instance.reshape([-1]), v2_instance.reshape([-1]), c)) < 1e-8
#         assert linalg.norm(log_map_x_v - log_map_x(v1_instance.reshape([-1]), v2_instance.reshape([-1]), c)) < 1e-8
# 
...
# 
#     x = x * 0.54321 / linalg.norm(x)
#     v = v / (linalg.norm(v) * lambda_x(x, c))
# 
#     t = 1e-6
#     d = (- unit_speed_geo(x, v, 0, c) + unit_speed_geo(x, v, t, c)) / t
# 
#     assert linalg.norm(d - v) < 1e-5
...
#     x = x * 0.54321 / linalg.norm(x)
# 
#     assert linalg.norm(log_map_x(x, exp_map_x(x, v, c), c) - v) < 1e-5
# 
#     r = np_random.random() * 10
#     assert linalg.norm(exp_map_x(0, r * log_map_x(0, x, c), c) - mob_scalar_mul(r, x, c)) < 1e-8
# 
...
#     x = np.random.rand(8)
#     x = x / linalg.norm(x) * 0.789
#     c = 1.0
#     assert linalg.norm(mob_mat_mul(M, x, c) - exp_map_x(0, M.dot(log_map_x(0, x, c)), c)) < 1e-5
#
#     for i in range(10):
#         c = random.random()
#         assert linalg.norm(mob_mat_mul(M, x, c) - exp_map_x(0, M.dot(log_map_x(0, x, c)), c)) < 1e-5
#         assert linalg.norm(mob_mat_mul(M, x, 1e-10) - M.dot(x)) < 1e-5
#
#         M_prime = np.random.rand(7, 5)
#         assert linalg.norm(mob_mat_mul(M_prime.dot(M), x, c) - mob_mat_mul(M_prime, mob_mat_mul(M, x, c), c)) < 1e-5
#
#         r = random.random() * 10
#         assert linalg.norm(mob_mat_mul(r * M, x, c) - mob_scalar_mul(r, mob_mat_mul(M, x, c), c)) < 1e-5
#
#         assert linalg.norm(mob_mat_mul(M, x, c) / linalg.norm(mob_mat_mul(M, x, c)) - M.dot(x) \
#                    / linalg.norm(M.dot(x))) < 1e-5
#
