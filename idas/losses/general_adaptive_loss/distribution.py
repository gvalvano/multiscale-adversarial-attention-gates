  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        x = tf.where(alpha < 4, (c(2.25) * alpha - c(4.5)) /
                     ...
                     c(5) / c(18) * utils.log_safe(c(4) * alpha - c(15)) + c(8))
        return x
...
def inv_partition_spline_curve(x):
    ...
    with tf.control_dependencies(assert_ops):
        alpha = tf.where(
            ...
            c(3.75) + c(0.25) * utils.exp_safe(x * c(3.6) - c(28.8)))
        return alpha
...
def log_base_partition_function(alpha):
  ...
  """
    ...
    with utils.get_resource_as_file(
            'idas/losses/general_adaptive_loss/data/partition_spline.npz') as spline_file:
...
def nllfun(x, alpha, scale):
  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        loss = static.lossfun(x, alpha, scale, approximate=False)
