...
from idas.losses.general_adaptive_loss import utils
from idas.losses.general_adaptive_loss import wavelet
...
def _construct_scale(x, scale_lo, scale_init, float_dtype):
    ...
    if scale_lo == scale_init:
        ...
        scale = utils.affine_softplus(latent_scale, lo=scale_lo, ref=scale_init)
    return scale
  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        if alpha_lo == alpha_hi:
            ...
            latent_alpha_init = utils.inv_affine_sigmoid(
            ...
            alpha = utils.affine_sigmoid(latent_alpha, lo=alpha_lo, hi=alpha_hi)
...
def lossfun_students(x, scale_lo=1e-5, scale_init=1.):
  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        ...
        scale = _construct_scale(x, scale_lo, scale_init, float_dtype)
        loss = utils.students_t_nll(x, tf.math.exp(log_df), scale)
...
def image_lossfun(x,
  ...
  Args:
    ...
    color_space: The color space that `x` will be transformed into before
      ...
      utils.rgb_to_syuv()). Note that changing this argument does not change the
  ...
  """
    ...
    with tf.control_dependencies(assert_ops):
        ...
        if representation in wavelet.generate_filters():
            ...
            x_stack = utils.image_dct(x_stack)
