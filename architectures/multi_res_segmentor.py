...
class SAUNet(object):
    ...
    def __init__(self, n_out, is_training, n_filters=64, n_out_att=None, use_bn=True, upsampling_mode='NN',
        ...
        Class for UNet architecture with self-attention. This is a 2D version (like the vanilla UNet), which means it
        only employs bi-dimensional convolution and strides. This implementation also uses batch normalization after
        each convolutional layer, and Attention Gates in the decoder.

        It is possible to condition these attention gates with an adversarial discriminator outside of this class,
        getting access to the tensors predicted for the self attention. See "Examples" below to see how to use it.
        ...
        Args:
            ...
            is_training (tf.placeholder(dtype=tf.bool) or bool): variable to define training or test mode; it is
                        needed for the behaviour of dropout, batch normalization, etc., which behave differently
